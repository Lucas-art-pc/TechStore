  ...
  <main class="flex-1 p-8">
    ...
    <form action="/add-product" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-xl shadow-md max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <textarea name="desc_prod" id="desc_prod" rows="4" required  class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"></textarea>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label for="preco_prod" class="block text-sm font-medium text-gray-700">Preço</label>
          <input type="number" step="0.01" name="preco_prod" id="preco_prod" required class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>

        <div>
          <label for="quantidade_prod" class="block text-sm font-medium text-gray-700">Quantidade</label>
          <input type="number" name="qtd_prod" id="qtd_prod" required class="mt-1 w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
        </div>
      ...
      <div>
        ...
        <input type="file" name="image_prod" id="image_prod" accept="image/*" required class="mt-1 w-full border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
      </div>

      <div class="text-center">
        <button type="submit" name="cadastra" id="cadastra" class="bg-blue-600 text-white cursor-pointer px-6 py-2 rounded-lg font-semibold hover:bg-blue-700 ">
          Salvar Produto
        </button>
      </div>
    </form>
